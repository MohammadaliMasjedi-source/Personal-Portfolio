...
> **Status:** building (deployed but pending Mo's content approvals before final publish) · **Realm:** personal / public presence · **Last updated:** 2026-06-30
...
- `.mc/project.json` + `PROJECT.md` — internal project record (machine + human readable).
...
- **Add FA/EN/DE + full art-life identity; keep the public tone humane/universal (2026-06-06).** Why: Mo wants a clean multilingual presence across his many lives. Notably, an unverified "Kiarostami award" line and all religious framing were *removed* as a precaution (under-claim until independent sources exist; Mo is an [personal-belief-statement-removed]). Wikipedia is to be pursued only via Articles-for-Creation once independent sources exist — never a self-written page.
...
- Two directories are gitignored and must never be published: `_private/` (Wikipedia drafts, facts, brand strategy) and `content/` (working content with do-not-publish flags / unverified claims).
...
- **Q:** What about the "Kiarostami award" / Wikipedia page? **A:** The unverified award line was removed until Mo confirms it with sources. A Wikipedia article would only be pursued via Articles-for-Creation once independent sources exist — never a self-written page (those get deleted).
...
- **Project record:** `.mc/project.json` (source of truth) and `PROJECT.md` (human companion); `LAUNCH-CHECKLIST.md` tracks the remaining Mo-only publish steps.
